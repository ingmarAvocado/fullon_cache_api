...
import pytest
from fullon_cache import AccountCache, BotCache, OrdersCache, TickCache, TradesCache
...
class TestOrderTradeAccountIntegration:
    ...
    async def test_order_fill_trade_position_update_flow(self, clean_redis):
        ...
        try:
            ...
            initial_position = Position(
                symbol="BTC/USDT", volume=0.0, price=0.0, cost=0.0, fee=0.0, ex_id="1"
            )
    ...
    async def test_multiple_orders_position_aggregation(self, clean_redis):
        ...
        try:
            # 1. Start with empty position
            position = Position(
                symbol="ETH/USDT", volume=0.0, price=0.0, cost=0.0, fee=0.0, ex_id="1"
            )
            ...
            assert (
                abs(eth_position.volume - 0.6) < 0.001
            )  # 0.1 + 0.2 + 0.3 (handle floating point precision)
...
class TestBotCacheIntegration:
    ...
    async def test_multi_bot_multi_symbol_coordination(self, clean_redis):
        ...
        try:
            ...
            for i, (bot, symbol) in enumerate(zip(bots, symbols, strict=False)):
            ...
            for bot, symbol in zip(bots, symbols, strict=False):
