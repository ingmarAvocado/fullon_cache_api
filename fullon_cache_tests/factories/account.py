...
class PositionFactory:
    ...
    def create_profitable_position(
        self, symbol: str = "BTC/USDT", profit_percent: float = 10.0, **kwargs
    ) -> dict[str, Any]:
    ...
    def create_losing_position(
        self, symbol: str = "BTC/USDT", loss_percent: float = 5.0, **kwargs
    ) -> dict[str, Any]:
    ...
    def create_leveraged_position(
        self, leverage: float = 10.0, **kwargs
    ) -> dict[str, Any]:
...
class AccountFactory:
        ...
        if result["total_value"] is None:
            ...
            for balance_data in result["balances"].values():
                if isinstance(balance_data, dict):
                    total += balance_data.get("total", 0) * self._get_usd_price(
                        balance_data
                    )
    ...
    def create_with_positions(
        self, positions: list[dict[str, Any]] = None, **kwargs
    ) -> dict[str, Any]:
        ...
        if positions is None:
            ...
            positions = [
                self.position_factory.create_profitable_position(
                    symbol="BTC/USDT", amount=0.5, profit_percent=5.0
                ),
                self.position_factory.create_losing_position(
                    symbol="ETH/USDT", amount=10.0, loss_percent=2.0
                ),
            ]
        ...
        return self.create(
            positions=positions_dict, pnl=total_pnl, used_margin=used_margin, **kwargs
        )

    def create_margin_account(
        self, leverage: float = 5.0, margin_level: float = 150.0, **kwargs
    ) -> dict[str, Any]:
        ...
        positions = [
            self.position_factory.create_leveraged_position(
                symbol="BTC/USDT", leverage=leverage, amount=1.0
            ),
            self.position_factory.create_leveraged_position(
                symbol="ETH/USDT", leverage=leverage, amount=10.0, price=3000.0
            ),
        ]
    ...
    def create_empty_account(self, **kwargs) -> dict[str, Any]:
        ...
        """
        return self.create(
            balances={}, positions={}, total_value=0.0, equity=0.0, pnl=0.0, **kwargs
        )

    def create_multi_currency_account(
        self, currencies: list[str] = None, **kwargs
    ) -> dict[str, Any]:
        ...
        return self.create(balances=balances, **kwargs)
