...
import pytest
from fullon_cache import OrdersCache, TickCache, TradesCache
from fullon_orm.models import Order, Tick
...
class TestTickerPerformance:
    ...
    async def test_ticker_update_performance(self, clean_redis):
        ...
        try:
            ...
            print("Ticker Update Performance:")
            ...
            assert (
                avg_time < 30.0
            ), f"Average ticker update time {avg_time:.2f}ms exceeds 30ms threshold"
            assert (
                median_time < 25.0
            ), f"Median ticker update time {median_time:.2f}ms exceeds 25ms threshold"
            assert (
                max_time < 150.0
            ), f"Max ticker update time {max_time:.2f}ms exceeds 150ms threshold"
    ...
    async def test_ticker_retrieval_performance(self, clean_redis):
        ...
        try:
            ...
            print("Ticker Retrieval Performance:")
            print(f"  Average: {avg_time:.2f}ms")
            print(f"  Median: {median_time:.2f}ms")
            print(f"  Max: {max_time:.2f}ms")
            ...
            assert (
                avg_time < 15.0
            ), f"Average ticker retrieval time {avg_time:.2f}ms exceeds 15ms threshold"
            assert (
                median_time < 10.0
            ), f"Median ticker retrieval time {median_time:.2f}ms exceeds 10ms threshold"
    ...
    async def test_bulk_ticker_operations(self, clean_redis, worker_id):
        ...
        try:
            ...
            symbols = [
                f"SYMBOL_{worker_id}_{i}/USDT" for i in range(50)
            ]  # Reduced from 100
            ...
            assert (
                per_update_time < 100.0
            ), f"Per-ticker update time {per_update_time:.2f}ms exceeds 100ms threshold"
            ...
            print(
                f"Bulk Ticker Retrievals ({successful_retrievals}/{len(symbols)} symbols):"
            )
            ...
            # Relaxed threshold for parallel execution
            assert (
                per_retrieval_time < 100.0
            ), f"Per-ticker retrieval time {per_retrieval_time:.2f}ms exceeds 100ms threshold"
...
class TestOrderPerformance:
    ...
    async def test_order_queue_performance(self, clean_redis, worker_id):
        ...
        try:
            ...
            if avg_queue_time > 0:
                ...
                ), f"Average queue time {avg_queue_time:.2f}ms exceeds 100ms threshold"
            if avg_dequeue_time > 0:
                assert (
                    avg_dequeue_time < 100.0
                ), f"Average dequeue time {avg_dequeue_time:.2f}ms exceeds 100ms threshold"
    ...
    async def test_order_data_performance(self, orders_cache):
        ...
        try:
            ...
            print("Order Data Performance:")
            print(f"  Average save time: {avg_save_time:.2f}ms")
            print(f"  Average retrieve time: {avg_retrieve_time:.2f}ms")

            assert (
                avg_save_time < 15.0
            ), f"Average save time {avg_save_time:.2f}ms exceeds 15ms threshold"
            assert (
                avg_retrieve_time < 15.0
            ), f"Average retrieve time {avg_retrieve_time:.2f}ms exceeds 15ms threshold"
...
class TestConcurrentPerformance:
    ...
    async def test_concurrent_ticker_updates(self, clean_redis):
        ...
        try:
            ...
            tasks = [update_tickers(i * 20, 20) for i in range(5)]
            ...
            print("Concurrent Ticker Updates (5 tasks × 20 updates):")
            print(f"  Total time: {total_time:.2f}ms")
            print(f"  Total operations: {total_operations}")
            print(f"  Average per operation: {avg_time:.2f}ms")
            print(
                f"  Operations per second: {1000 * total_operations / total_time:.0f}"
            )

            assert (
                avg_time < 110.0
            ), f"Average concurrent update time {avg_time:.2f}ms exceeds 110ms threshold"
    ...
    async def test_mixed_operations_performance(self, clean_redis):
        ...
        try:
            ...
            ticker_times, order_times, trade_times = await asyncio.gather(
                ticker_worker(), order_worker(), trade_worker()
            )
            end_total = time.perf_counter()
            ...
            print("Mixed Operations Performance (3 workers × 30 ops):")
            print(f"  Total time: {total_time:.2f}ms")
            print(f"  Total operations: {total_operations}")
            ...
            assert (
                mean(ticker_times) < 50.0
            ), "Ticker operations too slow under concurrent load"
            assert (
                mean(order_times) < 50.0
            ), "Order operations too slow under concurrent load"
            assert (
                mean(trade_times) < 50.0
            ), "Trade operations too slow under concurrent load"
...
class TestMemoryPerformance:
    ...
    async def test_model_memory_overhead(self, clean_redis):
        ...
        try:
            ...
            assert (
                tick_size < 1024
            ), f"Single Tick model size {tick_size} bytes exceeds 1KB"
            ...
            assert (
                per_model_time < 1.0
            ), f"Model creation time {per_model_time:.3f}ms per model too slow"
    ...
    async def test_large_dataset_performance(self, tick_cache):
        ...
        assert (
            per_store_time < 150.0
        ), f"Large dataset store time {per_store_time:.2f}ms per symbol too slow"
        assert (
            per_retrieve_time < 100.0
        ), f"Large dataset retrieve time {per_retrieve_time:.2f}ms per symbol too slow"

        # Under parallel stress, we expect significant degradation - accept 50% minimum
        min_expected = int(num_symbols * 0.5)
        assert (
            retrieved_count >= min_expected
        ), f"Too few symbols retrieved: {retrieved_count}/{num_symbols} (expected at least 50% under parallel stress)"

        # If we get less than 70%, this is expected under parallel stress - just warn
        if retrieved_count < int(num_symbols * 0.7):
            print(
                f"WARNING: Retrieved only {retrieved_count}/{num_symbols} ({retrieved_count/num_symbols*100:.1f}%) - Redis under parallel stress"
            )
