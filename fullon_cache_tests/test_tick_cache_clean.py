...
class TestCleanTickCache:
    ...
    async def test_get_ticker_not_found(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            symbol = symbol_factory.create(
                symbol="NONEXISTENT/USDT", exchange_name="binance"
            )
    ...
    async def test_get_any_ticker_cross_exchange(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            symbol_binance = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="binance", cat_ex_id=1
            )
            symbol_kraken = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="kraken", cat_ex_id=2
            )
    ...
    async def test_get_all_tickers_by_exchange_name(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            btc_binance = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="binance", cat_ex_id=1
            )
            eth_binance = symbol_factory.create(
                symbol="ETH/USDT", exchange_name="binance", cat_ex_id=1
            )
            btc_kraken = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="kraken", cat_ex_id=2
            )

            # Set tickers
            await cache.set_ticker(
                self.create_test_tick(btc_binance, "binance", 50000.0)
            )
            await cache.set_ticker(
                self.create_test_tick(eth_binance, "binance", 3000.0)
            )
    ...
    async def test_get_all_tickers_by_cat_ex_id(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            symbol1 = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="binance", cat_ex_id=1
            )
            symbol2 = symbol_factory.create(
                symbol="ETH/USDT", exchange_name="binance", cat_ex_id=1
            )
            symbol3 = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="kraken", cat_ex_id=2
            )
    ...
    async def test_delete_exchange_tickers(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            btc_symbol = symbol_factory.create(
                symbol="BTC/USDT", exchange_name="binance", cat_ex_id=1
            )
            eth_symbol = symbol_factory.create(
                symbol="ETH/USDT", exchange_name="binance", cat_ex_id=1
            )

            # Set tickers
            await cache.set_ticker(
                self.create_test_tick(btc_symbol, "binance", 50000.0)
            )
    ...
    async def test_set_ticker_updates_time_index(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            old_tick = self.create_test_tick(
                symbol, "binance", 50000.0, timestamp=1000.0
            )
            await cache.set_ticker(old_tick)

            # Set ticker with newer timestamp
            new_tick = self.create_test_tick(
                symbol, "binance", 51000.0, timestamp=2000.0
            )
    ...
    @pytest.mark.asyncio
    async def test_get_next_ticker_pubsub_integration(
        self, clean_redis, symbol_factory
    ):
        ...
        try:
            ...
            symbol = symbol_factory.create(
                symbol=unique_symbol, exchange_name="binance"
            )
            ...
            try:
                ...
                next_tick = await asyncio.wait_for(
                    cache.get_next_ticker(symbol), timeout=2.0
                )
                assert next_tick is not None
    ...
    async def test_concurrent_operations(self, clean_redis, symbol_factory):
        ...
        try:
            ...
            for i in range(5):
                symbol = symbol_factory.create(
                    symbol=f"TEST{i}/USDT", exchange_name="binance", cat_ex_id=1
                )
            ...
            assert (
                success_count >= 3
            )  # At least 3 should succeed under normal conditions
