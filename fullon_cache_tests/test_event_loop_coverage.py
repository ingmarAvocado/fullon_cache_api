...
class TestEventLoopManagerCoverage:
    ...
    def test_configure_auto_policy(self):
        ...
        assert manager.get_active_policy() in [
            EventLoopPolicy.UVLOOP,
            EventLoopPolicy.ASYNCIO,
        ]
    ...
    def test_configure_forced_policy_failure(self):
        ...
        with patch.object(manager, "_is_uvloop_available", return_value=False):
            with patch.object(
                manager,
                "_configure_uvloop",
                side_effect=ImportError("uvloop not available"),
            ):
                with pytest.raises(
                    RuntimeError, match="Failed to configure forced policy"
                ):
                    manager.configure()
    ...
    def test_configure_with_exception_fallback(self):
        ...
        with patch.object(
            manager, "_configure_uvloop", side_effect=Exception("Config failed")
        ):
            result = manager.configure()
...
class TestEdgeCases:
    ...
    def test_import_error_handling(self):
        ...
        with patch.object(
            manager,
            "_configure_uvloop",
            side_effect=ImportError("No module named 'uvloop'"),
        ):
