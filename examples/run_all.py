...
async def main():
    ...
    parser.add_argument(
        "--quick", 
        action="store_true", 
        help="Run shorter/faster versions of examples"
    )
    parser.add_argument(
        "--only", 
        nargs="*", 
    ...
    examples = [
        ...
        },
        {
            ...
            "args": ["--operations", "streaming" if args.quick else "all", 
                    "--duration", "5" if args.quick else "10"],
        ...
        },
        {
            ...
            "args": ["--operations", "status" if args.quick else "all",
                    "--duration", "5" if args.quick else "10"],
        ...
        },
        {
            ...
            "args": ["--operations", "basic" if args.quick else "all",
                    "--orders", "20" if args.quick else "50"],
    ...
    if args.list:
        ...
        print(f"\nUsage examples:")
        print(f"   python run_all.py --only tick_cache")
        print(f"   python run_all.py --only tick_cache bot_cache --quick")
        print(f"   python run_all.py --exclude process_cache --verbose")
        return
    ...
    if args.only:
        ...
        examples = [ex for ex in examples 
                   if any(only in ex["script"] for only in only_set)]
        if not examples:
