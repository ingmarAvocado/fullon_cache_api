        ...
        try:
            async for message in websocket:
                try:
                    ...
                    logger.error(
                        "Request handling failed: client_id=%s, error=%s",
                        client_id,
                        str(e),
                    )
                    error_response = {
...
class TickerWebSocketClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.error(
                "Failed to connect to WebSocket server: url=%s, error=%s",
                self.ws_url,
                str(e),
            )
    ...
    async def set_ticker(
        self, symbol: str, exchange: str, price: float, volume: float = 0
    ) -> bool:
        """Set ticker data."""
        params = {
            "symbol": symbol,
    ...
    async def stream_tickers(
        ...
        logger.info(
            "Starting ticker stream: exchange=%s, symbols=%s", exchange, symbols
        )
...
async def basic_ticker_operations(
    client, exchanges: list[str], verbose: bool = False
) -> bool:
    ...
    try:
        ...
        for exchange in exchanges:
            for symbol in symbols:
                ...
                await client.set_ticker(
                    symbol, exchange, current_price, volume=random.uniform(100, 1000)
                )

        print(
            f"🔄 Getting ticker data for {len(symbols)} symbols on {len(exchanges)} exchanges..."
        )
...
async def streaming_ticker_demo(
    client, symbols: list[str], duration: int = 15, verbose: bool = False
) -> bool:
    ...
    try:
        ...
        async for ticker_update in client.stream_tickers("binance", symbols):
            ...
            if verbose:
                print(
                    f"   📊 {ticker_update['symbol']}: ${ticker_update['price']:,.2f} "
...
async def run_demo(args) -> bool:
    ...
    try:
        ...
        async with client:
            ...
            # Parse parameters
            exchanges = (
                [e.strip() for e in args.exchanges.split(",")]
                if args.exchanges
                else ["binance", "kraken"]
            )
            symbols = (
            ...
            if args.operations in ["basic", "all"]:
                results["basic"] = await basic_ticker_operations(
                    client, exchanges, args.verbose
                )

            if args.operations in ["streaming", "all"]:
                results["streaming"] = await streaming_ticker_demo(
                    client, symbols, args.duration, args.verbose
                )
