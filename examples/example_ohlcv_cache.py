...
class MockOHLCVWebSocketAPI:
    ...
    async def stream_ohlcv_updates(self, symbol: str, timeframe: str) -> AsyncIterator[Dict[str, Any]]:
        ...
        for i in range(12):
            ...
            yield {
                ...
                "symbol": symbol,
                "timeframe": timeframe,
                "bar": [timestamp, open_price, high_price, low_price, close_price, volume],
                "update_id": i
            }
        ...
        async with fullon_cache_api() as handler:
            ...
            for symbol in symbols:
                for timeframe in timeframes:
                    bars = await handler.get_latest_ohlcv_bars(symbol, timeframe, count=50)
                    total_bars += len(bars)
                    
                    if verbose and bars:
                        latest_bar = bars[-1]
                        print(f"   🕯️ {symbol} {timeframe}: {len(bars)} bars, "
...
async def streaming_demo(symbols: List[str], timeframes: List[str], verbose: bool = False) -> bool:
...
async def run_demo(args) -> bool:
    ...
    timeframes = [t.strip() for t in args.timeframes.split(",")] if args.timeframes else ["1m", "5m"]
...
def main():
    ...
    parser.add_argument("--timeframes", default="1m,5m", help="Comma-separated timeframes")
