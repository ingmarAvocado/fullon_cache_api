...
class MockAccountWebSocketAPI:
    ...
    async def get_user_balance(self, exchange_id: int, currency: str) -> Optional[Dict[str, float]]:
        """Get specific currency balance via WebSocket."""
        await asyncio.sleep(0.02)
        balances = await self.get_user_balances(exchange_id)
        return balances.get(currency)

    async def get_position(self, exchange_id: int, symbol: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_portfolio_summary(self, exchange_id: int) -> Dict[str, Any]:
        ...
        total_balance_usd = sum(
            bal["total"] * random.uniform(20000, 50000) if curr == "BTC" else
            bal["total"] * random.uniform(2000, 4000) if curr == "ETH" else
            bal["total"]
    ...
    async def stream_balance_updates(self, exchange_id: int) -> AsyncIterator[Dict[str, Any]]:
    ...
    async def stream_position_updates(self, exchange_id: int) -> AsyncIterator[Dict[str, Any]]:
...
async def basic_account_operations(account_count: int = 3, verbose: bool = False) -> bool:
    ...
    try:
        async with fullon_cache_api() as handler:
            ...
            for symbol, position in positions.items():
                ...
                if verbose:
                    print(f"   {pnl_symbol} {symbol}: {position['volume']:.8f} @ "
        ...
        async with fullon_cache_api() as handler:
            ...
            async def balance_monitor():
                ...
                async for update in handler.stream_balance_updates(exchange_id):
                    ...
                    if verbose:
                        print(f"   💰 {change_symbol} {update['currency']}: "
            ...
            async def position_monitor():
                ...
                async for update in handler.stream_position_updates(exchange_id):
                    ...
                    if verbose:
                        print(f"   📈 {symbol} {update['symbol']}: {update['action']} "
            ...
            print(f"✅ Streaming completed: {balance_count} balance updates, "
